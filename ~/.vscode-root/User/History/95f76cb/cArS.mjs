...
async function backupServer(dummyMessage) {
  ...
  const backupData = {
    ...
    roles: Array.from(guild.roles.cache.values())
      ...
      .map((r) => ({
        ...
        color: r.hexColor,
        hoist: r.hoist,
        permissions: r.permissions.bitfield, // BigInt may be present here
  ...
  // Use a replacer function to serialize BigInts
...
async function start() {
  ...
  // Prompt for Discord token; while logging in, you can type
  ...
  // Once the client is ready, show the main menu
