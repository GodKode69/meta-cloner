...
async function backupServer(dummyMessage) {
  ...
  // Sanitize the guild name: remove emojis and special characters, trim spaces, and replace spaces with underscores
...
// Lists backup files from backups folder, extracts the server ID from the file name,
// and starts the restoration process on that server.
async function restoreBackup(dummyMessage) {
  ...
  // Simulate restoration process (actual logic would recreate channels, roles, etc.)
...
// MAIN MENU: Prompt user to select a command
async function mainMenu() {
  ...
  // Ask user if they want to execute another command
...
// MAIN: Prompt for token and then display the main menu
async function start() {
  setTerminalTitle();

  // Use password input with a mask to avoid accidental characters
  ...
  // Initiate login
  client.login(token);
  // Wait until the client is fully ready before proceeding
  await new Promise((resolve) => client.once("ready", resolve));
  console.log(`Logged in as ${client.user.tag}`);

  // Now that the client is ready, present the main menu for further input.
  await mainMenu();
}
